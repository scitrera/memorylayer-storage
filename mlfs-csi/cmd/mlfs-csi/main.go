...
func main() {
	...
	maxBackground := flag.Int("max-background", 0, "per-NODE mlfs FUSE -max-background (concurrent readahead/in-flight requests; 0 = mlfs default 64). Raise to 128-256 on read-heavy node pools, e.g. GPU model loading, to keep more parallel reads in flight")
